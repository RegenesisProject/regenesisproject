...
export const KeynotesPage: React.FC<KeynotesPageProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'keynotes' | 'speaker-kit'>('keynotes');
      ...
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
        ...
        <section className="text-center p-8 sm:p-12 rounded-3xl bg-[#06080F]/92 backdrop-blur-lg border border-[#D4AF37]/50 shadow-[0_20px_50px_rgba(0,0,0,0.9)] relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 max-w-md mx-auto">
            ...
            <button
              onClick={handleOpenSpeakerKit}
              className={`w-full sm:w-auto px-8 py-4 rounded-md border text-xs sm:text-sm uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center gap-2 font-extrabold ${
                activeTab === 'speaker-kit'
                  ? 'bg-[#D4AF37] text-black border-[#D4AF37]'
                  : 'bg-[#12141C] border-[#D4AF37]/60 hover:border-[#D4AF37] text-[#F3E5AB] hover:bg-[#1A1D28]'
              }`}
            >
        ...
        {/* TABS NAVIGATION */}
        <div className="flex items-center justify-center border-b border-[#D4AF37]/30 pt-4 pb-0 gap-4 sm:gap-8">
          <button
            onClick={() => setActiveTab('keynotes')}
            className={`pb-4 px-4 sm:px-8 text-xs sm:text-sm font-mono font-bold uppercase tracking-widest transition-all cursor-pointer border-b-2 ${
              activeTab === 'keynotes' 
                ? 'border-[#D4AF37] text-[#D4AF37] drop-shadow-[0_0_10px_rgba(212,175,55,0.5)]' 
          ...
          <button
            onClick={() => setActiveTab('speaker-kit')}
            className={`pb-4 px-4 sm:px-8 text-xs sm:text-sm font-mono font-bold uppercase tracking-widest transition-all cursor-pointer border-b-2 ${
              activeTab === 'speaker-kit' 
                ? 'border-[#D4AF37] text-[#D4AF37] drop-shadow-[0_0_10px_rgba(212,175,55,0.5)]' 
                : 'border-transparent text-gray-400 hover:text-white'
          ...
          >
            Request Speaker Kit Form
          </button>
        </div>

        {/* TAB 1: KEYNOTES OVERVIEW */}
        {activeTab === 'keynotes' && (
        ...
        {/* TAB 2: SPEAKER KIT FORM (SECTION 6B) */}
        {activeTab === 'speaker-kit' && (
          <div className="bg-[#06080F]/94 backdrop-blur-lg border border-[#D4AF37]/50 rounded-3xl p-6 sm:p-10 lg:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.9)] animate-fadeIn space-y-10">
              ...
              <div className="lg:col-span-6 bg-[#0E111B] border border-[#D4AF37]/40 p-6 sm:p-8 rounded-2xl shadow-xl text-left">
                {formSubmitted ? (
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-4 font-sans text-xs sm:text-sm">
                    ...
                    <button
                      type="submit"
                      className="w-full h-11 rounded-lg bg-gradient-to-r from-[#D4AF37] via-[#C5A028] to-[#9A7B2C] text-[#090A0C] font-extrabold text-xs uppercase tracking-widest flex items-center justify-center gap-2 cursor-pointer shadow-[0_4px_20px_rgba(212,175,55,0.3)] transition-all hover:brightness-110 active:scale-98"
                    >
